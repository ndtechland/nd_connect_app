...
class ReAssignTasks extends StatelessWidget {
  const ReAssignTasks({super.key});

  @override
  Widget build(BuildContext context) {
    final TaskHistoryController controller = Get.put(TaskHistoryController());

    print("Re-AssignTasks");
              ...
              builder: (context, constraints) {
                ...
                return Container(
                  ...
                  child: ListView.builder(
                    itemCount: controller.taskReassign.length,
                    itemBuilder: (context, index) {
                      final task = controller.taskReassign[index];
                      return  GestureDetector(
                        onTap: () async{
                          await controller.fetchTaskHisDetailData(task.id??0);
                          print(" iddReassign:${task.id}");
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Container(
                                      ...
                                      child: Icon(Icons.refresh_outlined,color: Colors.white,size: 25,),
                                    ),
                                    Expanded(
                                      child: Container(
                                        // color: Colors.pink,
